...
This covers `sa-module-v2/`, the only part of the shared materials with runnable code. KUQ, UnknownBench, and the Gemini/Qwen3 runs are discussed from their result files only — no source code for those was shared.
...
It assumes the forty-phrase keyword list reliably distinguishes genuine uncertainty from confident hedging — and trusts the same detector as both evaluation ground truth and training signal. It assumes comparing `with_memory` against `with_memory_sa` isolates SA's contribution, which is not quite right given that retrieval depth differs between the two conditions (more on this in §4.3). It assumes a single F1 at one operating point is sufficient to call a model better calibrated — the selective-prediction literature generally prefers a risk-coverage curve for exactly this reason (Kamath, Jia & Liang, 2020). And it assumes the train/test split and the unanswerable label set are clean, which §4.6 shows does not fully hold.

---

## 4. Problems, weak points, methodological risks

### 4.1 The gain looks like over-abstention, not better discrimination
...
A likely mechanism: the lessons and guidelines injected into the prompt are phrased using the same hedge vocabulary the detector looks for. A primed model can satisfy the scorer by echoing surface form rather than reasoning correctly. §6a and §6b test this directly.
...
### 4.3 Two of the four headline datasets have no code

The README lists `run_unknownbench.py` and `run_kuq.py`. Neither exists. The two largest claimed gains — KUQ (+0.138) and UnknownBench (+0.271) — cannot be inspected, reproduced, or verified from the shared materials.

### 4.4 Several result files never appear in the README

Going through the checkpoints folder surfaced runs that are not written up:

- `abstentionbench_summary.json`: F1 of zero in every condition. The model abstained exactly 0 out of 300 times regardless of condition. This matters because AbstentionBench (Meta FAIR, arXiv:2506.09038) is a current benchmark built specifically for answer-or-abstain, with KUQ as one of its own sub-benchmarks.
- `hle_summary.json`: accuracy bit-identical across all three conditions.
- `awarebench_summary.json`: RB+SA performs worse than baseline.
- `selfaware_v3_summary.json`: reruns the same benchmark to a different F1 (0.524 vs. the reported 0.506), with an extra condition that underperforms everything else.

None of this makes the reported numbers false. But "11/12 improved" undercounts how many combinations were actually run.

### 4.5 A likely regression in the unreported runs

The Gemini checkpoints folder includes a FalseQA result: F1 falling from 0.443 to 0.391 under RB+SA. This is not in the README.

### 4.6 Label quality issues in SelfAware

The SelfAware unanswerable set is mostly genuine subjective or philosophical questions, but includes corrupted GSM8K word problems — one switches from "bears" to "the dogs" mid-sentence — that are unanswerable only because the sentence does not parse. These are not the same kind of unanswerability the system is meant to learn.

### 4.7 Smaller issues

- Setup instructions do not reproduce as written: flask vs. llama-server naming inconsistencies, a `simple_server.py` and a `download_selfaware` module that both do not exist.
- The factual-correctness check uses a loose substring match. It does not touch the headline F1 but corrupts which lessons get extracted.
- Every episode's confidence is hardcoded to 0.5, yet `SelfModel` computes a calibration gap from it regardless — meaninglessly.
- Nine failure types are declared in `memory.py`; only four are ever produced.
- SelfAware train and test sets have zero overlapping questions. FalseQA has one duplicate out of 2838/1892 — negligible but not literally zero.
...
The from-scratch numbers sit close to the reported 0.425 / 0.483 / 0.506, so the effect direction reproduces. But RB+SA beats the scrambled placebo by only 0.029 F1 — not distinguishable from noise at n=500. RB+SA against baseline is significant, but that is consistent with general content priming rather than relevant retrieval. One caveat: scrambling only touched per-episode retrieval. The twelve standing guidelines were identical in both conditions, so this does not rule out the guidelines carrying real weight.
...
McNemar: RB+SA vs. scrambled p=0.719 (not significant); RB+SA vs. baseline p=1.000 — not distinguishable from doing nothing at all; RB vs. baseline p=0.017, the only comparison that clears significance.

Across both runnable datasets, the claim that relevant retrieval drives the gain does not survive direct testing. A real, modest effect of adding memory-derived content in general does appear to be present — but relevance to the specific question does not seem to be what matters.
...
One way to break out of that loop is to use a model-internal uncertainty signal that does not depend on what the model says out loud. Semantic entropy (Farquhar et al., *Nature* 2024) samples several generations and computes entropy over semantically clustered answers rather than surface wording — directly measuring whether the model is generating consistent answers or spreading probability across different meanings. Semantic Entropy Probes (Kossen et al., 2024) approximate this cheaply from a single generation's hidden states, without the cost of multiple samples.
...
- Has an internal ablation ever compared retrieval-relevant against retrieval-irrelevant SA content? My placebo could not statistically distinguish the two on either runnable dataset. Do KUQ and UnknownBench tell a different story?
- Where is the code for KUQ, UnknownBench, and the Gemini/Qwen3 runs?
- Were `abstentionbench`, `hle`, `awarebench`, and `selfaware_v3` left out of the README because they are known-broken, or simply not yet written up?
- Is abstain-class F1 really the right target, or would something that penalises over-abstention symmetrically — a risk-coverage curve, or accuracy at a fixed abstention rate — better capture what "more trustworthy" is meant to mean here?

---

## AI tool usage

Used Claude (Claude Code) throughout — reading the code and result files, running literature searches, implementing the ablation, running the experiments, and drafting this report. Every finding was independently verified against the actual files and logs. The choice of what to check and how to interpret the results was mine.
